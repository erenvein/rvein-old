...
client.on("channelDelete", async channel => {
  ...
  modlog.findOne({
    ...
    .setAuthor(`${channel.type.replace("n", "N").replace("t", "T").replace("v", "V")} Channel "${channel.name}" deleted by ${m.user.tag}`)
    .setColor(client.logcolor)
    .setDescription(`**Name** ${channel.name}\n**Type** ${channel.type}\n**Category** ${channel.parent || "None"}\n**Position** ${channel.rawPosition}\n**Created at** ${huma(Date.now() - channel.createdTimestamp)} ago`)
    .setTimestamp()
